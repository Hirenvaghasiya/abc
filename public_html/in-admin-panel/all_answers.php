...
<div id="main_container">
    ...
    <div class="right_header">Welcome Admin, <a href="http://www.ask4u.com/">Visit site</a> | <a href="#" class="messages">(3) Messages</a> | <a href="#" class="logout">Logout</a></div>
    <div id="clock_a"></div>
    </div>
...
<table id="rounded-corner" summary="2007 Major IT Companies' Profit">
    <thead>
    	<tr>
            ...
            <th scope="col" class="rounded">Answer ID</th>
            <th scope="col" class="rounded">Question ID</th>
            <th scope="col" class="rounded">User ID</th>
            <th scope="col" class="rounded">Answer Dec.	</th>
    ...
    <tbody>
    <?php
		$sel_ans="select * from answer order by ans_date desc ";
		$tmp=mysql_query($sel_ans) or die(mysql_error());
		while($res=mysql_fetch_assoc($tmp))
		{
	?>
    	<tr>
        	<td><input type="checkbox" name="" /></td>
            <td><?php echo $res['ans_id']; ?></td>
             <td><?php echo $res['que_id']; ?></td>
              <td><?php echo $res['user_id']; ?></td>
               <td><?php echo $res['ans_des']; ?></td>
            
            <td><a href="all_answers.php?id=<?php echo $res['ans_id'];?>&op=edit_ans"><img src="images/user_edit.png" alt="" title="" border="0" /></a></td>
            <td><a href="all_answers.php?id=<?php echo $res['ans_id'];?>&op=delete_ans" class="ask"><img src="images/trash.png" alt="" title="" border="0" /></a></td>
        </tr>
        <?php
		}
		?>
     ...
     <h2></h2>
         ...
         <form action="" method="post" class="niceform">
                ...
                <fieldset>
                <?php
					if(isset($_GET['op']) && $_GET['op']=='edit_ans')
					{
						include("ans_update.php");
						if(isset($_POST['submit']))
						{
							$ans_id=$_POST['ans_id'];
							$ans=$_POST['ans'];
							$date=$_POST['date'];
							$upd_ans="update answer set ans_des='$ans', ans_date='$date' where ans_id=$ans_id";
							mysql_query($upd_ans) or die(mysql_error());
							unset($_GET['op']);
							header("location:all_answers.php?op=update");
						}
					}
					if(isset($_GET['op']) && $_GET['op']=='delete_ans')
					{
						$ans_id=$_GET['id'];
						$del_ans="delete from answer where ans_id=$ans_id";
						mysql_query($del_ans) or die(mysql_error());
						unset($_GET['op']);
						header("location:all_answers.php?op=delete");
					}
		 ...
		 <?php
			...
			{
				echo "Answer Successfully Deleted..";
			}
		 ?></div>
         <?php
			}
