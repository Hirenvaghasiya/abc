...
<li><a   href="All_questions.php"><u>All Answeres</a><!-- Begin Home Item -->
 ...
 <!--    <div class="search">
  ...
  <!-- Filter-->
    ...
    <div id="listing" style="height:100px;">
      <!--  ------------------------------------------   Asp.net  ---------------------------------------------------------------------------------- -->  
  <!--  -------------------------------------------------------------------------------------------------------------------------------------------- -->   
    <!-- #1 -->
    <div class="discounted-item job portfolio-four" style="height:200px;">		
    <ul class="tab-menu">
    <?php
		include("cnn.php");
	...
	?>
           <li>
          		<a class="ajaxpicture" href="uploads/<?php echo $res['user_id']?>.jpg"><img src="uploads/<?php echo $res['user_id']?>.jpg" alt=""/></a>
			...
			if($res['que_status']==1)
			{
			  	$sel_ans="select * from answer where que_id=(select que_id from question where que_status=1 and que_id=$res[que_id])";
				$tmp6=mysql_query($sel_ans) or die(mysql_error());
				$res2=mysql_fetch_assoc($tmp6)
				
			  ?>
              <a  href="view_answer.php?id=<?php echo $res2['ans_id']; ?>" class="middle-button" style="float:right; margin-left:80px; margin-bottom:8px; margin-top:-18px;">
 ...
 <!--   <div class="discounted-item job portfolio-four">		
   ...
   <!--  ------------------------------------------  PHP ---------------------------------------------------------------------------------- -->   
    ...
    <div class="discounted-item card portfolio-four">		
		...
		?>
           <li>
			<a class="ajaxpicture" href="uploads/<?php echo $res1['user_id']?>.jpg"><img src="uploads/<?php echo $res1['user_id']?>.jpg" alt=""/></a>        
         	 <span class="tab-date"><?php echo $res1['date']?></span>
				...
				<?php
			if($res1['que_status']==1)
			{
			  	$sel_ans="select * from answer where que_id=(select que_id from question where que_status=1 and que_id=$res1[que_id])";
				$tmp6=mysql_query($sel_ans) or die(mysql_error());
				$res2=mysql_fetch_assoc($tmp6)
