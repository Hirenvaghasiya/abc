...
<div class="dot" style="margin-top:296px;"></div>
...
<div class="container_16" >
  ...
  <!-- Random Poster Start-->
  <div id="random-poster" class="grid_16" style="margin-top:-600px; margin-left:100px;">
 
  <?php
  	$id=$_GET['id'];
	$qry="SELECT * FROM question WHERE que_id = ( SELECT que_id FROM answer WHERE ans_id =$id)";
	$tmp1=mysql_query($qry) or die(mysql_query());
	$res1=mysql_fetch_assoc($tmp1);
	$i=0;
	?>
	<p><?php echo "<h1>Que:-$res1[que]</h1>";?></p>
        <div style="height:500px; float:left; overflow-y:auto; width:900px; margin-top:-10px;">

	<?php
	$que_id=mysql_fetch_assoc(mysql_query("select * from answer where ans_id=$id"));
	$sel_que="select * from answer where que_id=$que_id[que_id]";
	$tmp=mysql_query($sel_que) or die(mysql_error());
	while($res=mysql_fetch_assoc($tmp))
	{
		$i=$i+1;
		echo "<h1>Ans:$i</h1> $res[ans_des]";
